...
/// formal_citation: "Andreasen & Andersen (1930), Kolloid-Z. 50, 217"
/// formal_form: "P(D) = (D^q - D_min^q) / (D_max^q - D_min^q), q in [0.30, 0.45]"
pub fn compute_packing_density<B: Backend>(
    coarse_fraction: Tensor<B, 2>,
    fine_fraction: Tensor<B, 2>,
) -> Tensor<B, 2> {
    // Empirical interaction model (de Larrard approximation)
    let total_agg = coarse_fraction
        .clone()
        .add(fine_fraction.clone())
        .clamp_min(1e-6);
    ...
    diff_sq.mul_scalar(-k).add_scalar(p_max).clamp_min(0.5)
}
