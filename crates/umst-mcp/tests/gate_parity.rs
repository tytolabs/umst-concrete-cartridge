...
//! GO-LIVE Step 2: package `default = ["rmcp-wire"]` pulls agent-layer → default list is **13**.
//! Gate `gate_check_mix_result` bytes are unchanged (runtime/default flip only).
//!
//! Run:
//! - default (13-tool list + gate + call frames): `cargo test -p umst-mcp --test gate_parity`
//! - base-four (`--no-default-features`): historical 4-tool list
...
/// `--no-default-features`: historical 4 base tools.
...
/// Default package features (rmcp-wire → agent-layer): tools/list must be the 13-tool set.
#[cfg(feature = "agent-layer")]
#[test]
fn tools_list_default_thirteen_names() {
    ...
    assert_eq!(
        expected.len(),
        13,
        "fixture default names (agent-native via rmcp-wire)"
    );
