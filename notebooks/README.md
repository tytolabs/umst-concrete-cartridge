...
| RC beam strut-and-tie | [`_run_beam_demo.sh`](_run_beam_demo.sh): [`optimize_rc_beam`](../crates/umst-concrete-cartridge/examples/optimize_rc_beam.rs) (default **32×8** grid, `UMST_BEAM_ITERS=90`, `UMST_BEAM_DUMP_STRIDE=3`; NPY under `examples/_artifacts/beam/`) → [`render_beam_gif.py`](render_beam_gif.py) → `notebooks/_artifacts/beam_strut_and_tie.gif`. Optional renderer env: `UMST_BEAM_GIF_MAX_SIDE`, `UMST_BEAM_GIF_SUPERSAMPLE`, `UMST_BEAM_GIF_FRAME_MS`, `UMST_BEAM_GIF_HOLD_MS`, `UMST_BEAM_GIF_HOLD_FRAMES` (see script header / `render_beam_gif.py`). Uses `numpy` + `pillow`; bootstraps `BEAM_DEMO_VENV` or `.venv_beam_gif`. If `cargo run … optimize_rc_beam` fails, falls back to [`beam_demo_synthetic_npys.py`](beam_demo_synthetic_npys.py). **Commit gate:** same `_artifacts` GIF hygiene as the Striatus shell row. |
