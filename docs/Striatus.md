...
- **Default CI quick (`shell_topology_rib_pattern_quick`):** the compact **9×8** slab hard-codes a **72 Pa** roof ramp with **r=0.38** (see `shell_topology_rib_pattern.rs`) — **not** the same nodal scale as the **`optimize_shell_3d`** **50 Pa** default — so **post–volume-projection** **greyness** \(\mathrm{mean}(4\rho(1-\rho))\) over the **full volume** stays below **0.15** within the **≤20**-outer CI budget (roof Heaviside **`xy_var`** remains the planar-texture gate).
