...
- Optional **roof traction ramp in x** (`UMST_SHELL_ROOF_RAMP=1`, strength `UMST_SHELL_ROOF_RAMP_F`, default **0.2**) biases the top-face **z** nodal load; **`optimize_shell_3d`** and the opt-in **`shell_topology_rib_pattern_full_v04`** harness use the **same** literal-**`1`** parsing (unset = uniform roof). The CI quick slab uses a fixed small ramp for texture (see `shell_topology_rib_pattern.rs`).
