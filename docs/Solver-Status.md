...
**Release smoke 1-outer H4 diag (2026-06-10, `UMST_SHELL_H4_DIAG=1`, `RIB_FULL_ITERS=1`, manifold e9c6564):** Step C **pass**; **`grad_l2=0`**, H5 forward finite audit clean, param grad audit empty; test **fail** VF band only. **20-outer smoke not run** (grad gate).
**Release smoke 20-outer (2026-06-10, `--release`, `MAX_CG=10000`):** **FAILED** Step C **`eq_rel_res`** gate — final **`eq_rel≈0.24`** vs tol **1e-6**; **`pcg_rel≈1e-6`** passes; **`pcg_iter` max ≈5236** (under cap). `rho_raw` barely leaves **[0.501,0.501]**; **`xy_var=0`**. Not the bar **0.94** floor — PCG/equilibrium residual metrics **diverge at Striatus N** (investigate before 200-outer). **λ_g forbidden.**
