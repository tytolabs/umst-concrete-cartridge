...
|--------|------|---------------|------------|-----------------------------------|
| `PowersGelParameters` | `crates/umst-concrete-cartridge/src/calibration.rs:40` | `lean://umst-formal/Lean/Powers.lean#PowersState` | thermodynamic_mix | physicalSecondLaw |
| `FormalBlock` | `crates/umst-concrete-cartridge/src/calibration.rs:54` | `lean://umst-formal/Lean/Gate.lean#Admissible` | umst.gate.cd_transition | physicalSecondLaw |
| `CalibrationMeta` | `crates/umst-concrete-cartridge/src/calibration.rs:81` | `lean://umst-formal/Lean/Powers.lean#S_intrinsic` | thermodynamic_mix | physicalSecondLaw |
| `RegimeBounds` | `crates/umst-concrete-cartridge/src/calibration.rs:127` | `lean://umst-formal/Lean/OrderStatisticsBand.lean#order_statistic_concentration` | umst.cartridge.concrete.acceptance_band | NONE |
| `AcceptanceBlock` | `crates/umst-concrete-cartridge/src/calibration.rs:162` | `lean://umst-formal/Lean/OrderStatisticsBand.lean#p25_p75_admissibility` | umst.cartridge.concrete.acceptance_band | NONE |
| `regime_check_scalars` | `crates/umst-concrete-cartridge/src/calibration.rs:310` | `lean://umst-formal/Lean/RegimeSoundness.lean#warnings_empty_iff_in_regime` | umst.cartridge.concrete.regime | NONE |
| `any_bundled_profile_covers_scalars` | `crates/umst-concrete-cartridge/src/calibration.rs:396` | `lean://umst-formal/Lean/RegimeSoundness.lean#warnings_empty_iff_in_regime` | umst.cartridge.concrete.regime | NONE |
...
| `Profile` | `crates/umst-concrete-cartridge/src/calibration.rs:187` | `STRUCTURAL` | — | Parsed TOML aggregate routed by `bundle_id`; field invariants delegated to nested serde structs. |
| `RegimeViolation` | `crates/umst-concrete-cartridge/src/calibration.rs:204` | `STRUCTURAL` | — | Named-field regime violation records for CLI warning strings. |
| `load_bundled` | `crates/umst-concrete-cartridge/src/calibration.rs:257` | `STRUCTURAL` | — | Bundled `include_str!` loader with normalized bundle id validation. |
| `profile_descriptions` | `crates/umst-concrete-cartridge/src/calibration.rs:469` | `STRUCTURAL` | — | Static HashMap of tab-separated CLI profile blurbs (human-readable only). |
...
| `OpticalAuditWireV1` | `crates/umst-concrete-cartridge/src/facade/mod.rs:908` | `literature://optics/fresnel-simpson-portland-paste` | — | "ASTM E903 spectrum; Sihvola 1999 dielectric database; v0.4 Track H3 acceptance gate" \| "(R_solar, A_uv, ε_lwir) integrated from plain-Portland (λ, ε_r) profile at 50 mm" |
...
| `PHYSICS_PIPELINE_SCHEMA_VERSION` | `crates/umst-concrete-cartridge/src/lib.rs:55` | `literature://wire-schema-physics-pipeline-v1` | — | "physics_pipeline schema tag (`physics_pipeline.v1`)" \| "`schema_version` string on serde `PhysicsPipelineReport` — bump tag when breaking report shape." |
...
| `ProvenanceFormal` | `crates/umst-concrete-cartridge/src/calibration.rs:68` | `NONE` | — | Serde lift of TOML `[provenance.formal]`; `status` string is file metadata (may include Boundary scope), not a Rust `formal_status` bucket. |
| `CalibrationProvenance` | `crates/umst-concrete-cartridge/src/calibration.rs:101` | `NONE` | — | Dataset and Zenodo citation bundle parsed from TOML only; no Lean witness on this serde container — see `docs/FormalAnchors.md` “Future formal links” for manifold adjoint context. |
| `CalibrationModelSection` | `crates/umst-concrete-cartridge/src/calibration.rs:154` | `NONE` | — | Dispatch metadata only; Jennings gel-space monotone strength witness applies once `powers_compressive_strength_mpa` ships a Jennings branch (TODO_FORMAL note on that function). |
| `ContractBlock` | `crates/umst-concrete-cartridge/src/calibration.rs:179` | `NONE` | — | Contract metadata (`verification_status`); hyperbox regime warnings are soundness-witnessed on `regime_check_scalars` — see RegimeSoundness anchor there. |
| `CalibrationError` | `crates/umst-concrete-cartridge/src/calibration.rs:213` | `NONE` | — | Bundled profile IO and TOML parse failures only; DEC mass-conservation witness belongs on the manifold Laplacian — see `docs/FormalAnchors.md` “Future formal links”. |
| `load_from_path` | `crates/umst-concrete-cartridge/src/calibration.rs:265` | `NONE` | — | Filesystem path IO for non-bundled TOML; parse errors surface as CalibrationError. |
...
| `prediction_wire_v1` | `crates/umst-concrete-cartridge/src/facade/mod.rs:947` | `NONE` | — | Pure wire projection; transport encoding is caller-owned. |
| `prediction_wire_v2` | `crates/umst-concrete-cartridge/src/facade/mod.rs:969` | `NONE` | — | Pure wire projection; nested objects merged by CLI/MCP `serde_json`. |
| `MixSpecWireOut` | `crates/umst-concrete-cartridge/src/facade/mod.rs:1086` | `NONE` | — | Round-trip mix spec view for CLI `mix print` / MCP. |
| `mix_spec_wire_out` | `crates/umst-concrete-cartridge/src/facade/mod.rs:1099` | `NONE` | — | Serialize-friendly mix view without JSON crate in core. |
...
| `PhysicsPipelineSummary` | `crates/umst-concrete-cartridge/src/lib.rs:43` | `NONE` | — | Scalar digest accompanying report JSON. |
| `PipelineStageRecord` | `crates/umst-concrete-cartridge/src/lib.rs:47` | `NONE` | — | Stage record type embedded in [`PhysicsPipelineReport`]. |
| `PipelineStageStatus` | `crates/umst-concrete-cartridge/src/lib.rs:51` | `NONE` | — | Serialized stage disposition enum for MCP/CLI audit trails. |
| `GateEvaluator, ThermodynamicTransitionEvaluator, TransitionGateEvaluator` | `crates/umst-concrete-cartridge/src/lib.rs:61` | `NONE` | — | Re-exports host transition gate traits when `manifold-gate` is enabled. |
| `UmstManifest` | `crates/umst-concrete-cartridge/src/lib.rs:68` | `NONE` | — | Re-exports manifold deployment manifest when `manifold-manifest` is enabled. |
| `ros` | `crates/umst-concrete-cartridge/src/lib.rs:73` | `NONE` | — | Re-exports manifold ROS serde DTOs when `ros2-contract` is enabled. |
