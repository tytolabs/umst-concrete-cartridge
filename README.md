...
- **Accurate Thermal Curing:** We track the exact speed of the chemical reaction that hardens cement (known as **hydration kinetics**) using a classical thermal correction model (the **Arrhenius relation**):
  ...
  <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;\alpha(t)%20=%20\int_0^t%20k(T)%20\cdot%20f(\alpha)%20\,%20dt%20\quad%20\text{where}%20\quad%20k(T)%20=%20A%20\exp\left(-\frac{E_a}{R%20T}\right)" alt="\alpha(t) = \int_0^t k(T) \cdot f(\alpha) \, dt \quad \text{where} \quad k(T) = …"/></p>
...
- **Differentiable Carbon Tracking:** We calculate the carbon footprint directly from the material recipe. Because this carbon calculation is fully connected to our spatial mathematical gradients (making it **differentiable**), design algorithms can automatically discover the singular, optimal shape and recipe that minimizes greenhouse gases while guaranteeing the structure will not collapse:
  ...
  <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;GWP(\mathbf{w})%20=%20\mathbf{w}%20\cdot%20\mathbf{g}%20=%20\sum_{i=1}^n%20w_i%20g_i" alt="GWP(\mathbf{w}) = \mathbf{w} \cdot \mathbf{g} = \sum_{i=1}^n w_i g_i"/></p>
...
*   **Exact Tensor Formulation:** DLVO forces calculate early-stage slurry colloidal stability by summing electrostatic repulsion ($V_R$) and van der Waals attraction ($V_A$). The ITZ layer scales local mechanical stiffness via local porosity:
    ...
    <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;V_{\text{total}}%20=%20V_R%20+%20V_A%20=%202\pi\epsilon%20R%20\psi_0^2%20\ln\left(1%20+%20e^{-\kappa%20D}\right)%20-%20\frac{A_H%20R}{12%20D}" alt="V_{\text{total}} = V_R + V_A = 2\pi\epsilon R \psi_0^2 \ln\left(1 + e^{-\kappa D…"/></p>
    
    <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;E_{\text{ITZ}}%20=%20E_{\text{paste}}%20\cdot%20\left(%20\frac{1%20-%20\phi_{\text{ITZ}}}{1%20-%20\phi_{\text{paste}}}%20\right)^m" alt="E_{\text{ITZ}} = E_{\text{paste}} \cdot \left( \frac{1 - \phi_{\text{ITZ}}}{1 - …"/></p>
...
*   **Exact Tensor Formulation:** Models creep compliance $J(t, t_0)$ via a Kelvin-Voigt chain and shrinkage strain $\epsilon_{\text{sh}}$ via Kelvin-Laplace capillary tension:
    ...
    <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;J(t,%20t_0)%20=%20\frac{1}{E_0}%20+%20\sum_{i=1}^k%20\frac{1}{E_i}%20\left(%201%20-%20e^{-(t-t_0)/\tau_i}%20\right)" alt="J(t, t_0) = \frac{1}{E_0} + \sum_{i=1}^k \frac{1}{E_i} \left( 1 - e^{-(t-t_0)/\t…"/></p>
    
    <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;\epsilon_{\text{sh}}(t)%20=%20\epsilon_{\infty}%20\cdot%20\left(%201%20-%20\text{RH}(t)^n%20\right)%20\quad%20\text{where}%20\quad%20P_{\text{cap}}%20=%20-%20\frac{\rho%20R%20T}{M}%20\ln(\text{RH})" alt="\epsilon_{\text{sh}}(t) = \epsilon_{\infty} \cdot \left( 1 - \text{RH}(t)^n \rig…"/></p>
...
*   **Exact Tensor Formulation:** Simulates the localized deposition rate of precipitated calcite ($m_{\text{calcite}}$) along crack surfaces based on moisture transport:
    ...
    <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;\frac{d%20m_{\text{calcite}}}{d%20t}%20=%20k_{\text{precip}}%20\cdot%20a_{\text{crack}}%20\cdot%20\left(%20\frac{[\text{Ca}^{2+}][\text{CO}_3^{2-}]}{K_{sp}}%20-%201%20\right)%20\cdot%20\theta(\text{RH}%20-%20\text{RH}_{\text{crit}})" alt="\frac{d m_{\text{calcite}}}{d t} = k_{\text{precip}} \cdot a_{\text{crack}} \cdo…"/></p>
...
*   **Exact Tensor Formulation:** Evaluates printed layer buildability by tracking yield stress development ($\tau_y$) over print age ($t$) and calculating structural elastic buckling limits:
    ...
    <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;\tau_y(t)%20=%20\tau_{y0}%20+%20R_{\text{th}}%20\cdot%20t%20\quad%20\Longrightarrow%20\quad%20P_{\text{buckling}}%20=%20\frac{\pi^2%20E(t)%20I}{4%20H(t)^2}" alt="\tau_y(t) = \tau_{y0} + R_{\text{th}} \cdot t \quad \Longrightarrow \quad P_{\te…"/></p>
...
*   **Exact Tensor Formulation:** Calculates dynamic net carbon footprint by subtracting dynamic carbonation (sequestration) from the initial GWP:
    ...
    <p align="center"><img src="https://latex.codecogs.com/svg.image?%5Cdpi%7B150%7D&space;\text{Net%20CO}_2(t)%20=%20\sum%20w_i%20g_i%20-%20\int_A%20\int_0^x%20C_{\text{seq}}%20\cdot%20\text{erfc}\left(\frac{x}{2\sqrt{D_{\text{CO}_2}%20t}}\right)%20\,%20dx%20\,%20dA" alt="\text{Net CO}_2(t) = \sum w_i g_i - \int_A \int_0^x C_{\text{seq}} \cdot \text{e…"/></p>
