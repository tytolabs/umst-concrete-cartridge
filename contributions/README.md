...
6. Periodic: cat contributions/merged/*/*.jsonl | python3 scripts/ingest_contributions.py --db .umst-memory/memory.db
```
...
After merge, relocate inbox file to `contributions/merged/<YYYY-MM>/` and append manifest entry when `MANIFEST.jsonl` is introduced (Pass 1).
