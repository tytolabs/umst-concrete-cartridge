...
impl RheologyEngine {
    ...
    pub fn compute(mix: &MixTensor, packing_density: f32) -> RheologyResult {
        ...
        for i in 0..count {
            ...
            let shape = data[offset + 7]; // Shape factor (1=sphere, 0=angular)
        ...
        console::log_1(&format!(
            "[DEBUG H2e] water_vol={:.4}, solid_vol={:.4}, total_vol={:.4}, sp_raw={:.2}kg, binder={:.1}kg, sp_dosage={:.4} ({:.2}%)",
            water_vol, solid_vol, total_vol, sp_raw_mass, binder_mass, sp_dosage, sp_dosage * 100.0
        ).into());
        // #endregion
        if total_vol <= 0.0001 {
        ...
        //
        // PCE Superplasticizer effectiveness (literature-calibrated):
        // - At 0.3% dosage: ~45% reduction (factor ≈ 0.55)
        // - At 0.5% dosage: ~55% reduction (factor ≈ 0.45)
        // - At 1.0% dosage: ~75% reduction (factor ≈ 0.25)
        // - At 2.0% dosage: ~90% reduction (factor ≈ 0.10)
        let sp_factor = (-sp_dosage * 200.0).exp().max(0.05);
        ...
        let thixotropy_index = if base_thixotropy > 0.0 {
